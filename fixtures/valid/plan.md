...
**Goal**: Author `dot_local/share/leanplan/scripts/executable_sync-leanplan.sh` in chezmoi source per `DESIGN#Decision-1-sync-as-shell-script-in-canonical-scripts`. Script hard-codes the in-repo path per `DESIGN#Decision-2-in-repo-path-hard-coded`, re-inserts the runtime header note between title and body per `DESIGN#Decision-3-header-note-reinsertion-between-title-and-body`, invokes `chezmoi apply ~/.local/share/leanplan/leanplan.md` at the end per `DESIGN#Decision-4-chezmoi-apply-called-at-script-end`, and prints `already up-to-date <hash>` or `updated <prior> -> <new>` via `shasum -a 256` per `DESIGN#Decision-5-hash-check-via-shasum`. Error-exit before writing anything when the in-repo source is absent, satisfying `SPEC#O-3-missing-source-errors`.
...
- After making a whitespace edit in the in-repo source, running the script causes `shasum -a 256 ~/.local/share/leanplan/leanplan.md` to change to match the new in-repo body — verifies `SPEC#O-1-single-invocation-sync` and `SPEC#INV-1-directional`.
...
- Running the script twice in a row with no source change produces `already up-to-date <hash>` on the second run — verifies `SPEC#O-2-staleness-reporting`.
...
| O-2 staleness-reporting | S1 Completion ("already up-to-date on second run") |
