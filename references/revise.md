...
- The justification — a `Delta-<N>` block in `understanding-shifts.md` (`artifact-contract.md` → Understanding Shifts). Either named at invocation (the `/sharpen` handoff, or a hand-up from an implementation stop-the-line) or recorded at intake from the planner's asserted drift before any edit. With no Delta and no recordable justification, stop without mutating.
