...
8. **Self-check**:
   ...
   - Orthogonality pass — each task card carries one concern no sibling task does; resolve any pair across the round that shares one (*One Concern Per Item*, `artifact-contract.md`).
...
- **Process specifics belong here; tech-realization specifics belong in Design (the Product↔Process cut, `framework-design.md` §2).** A task card describes the *work* — what outcome it achieves, how to verify it, in what work-stance. The *finished system's shape* (field mappings, response/proto shapes, controller orchestration sequences, signatures, code paths, schemas) belongs in a Design `D-<N>` block — the symmetric guard with Design's "tech-realization specifics live here". Anchor the Decision (`Design#D-<N>-<slug>`) from the Goal; do not restate it.
...
- **`**GAP**` ack is rare.** Use it only for deliberately-deferred coverage with a documented acceptance rationale; the validator then treats the marked item as acknowledged, not a coverage error.
...
- **Goal**: <process-framed — what this task achieves + how (when non-obvious); inline anchors like `Spec#B-1-…` and `Design#D-1-…`.
  Anchor — don't restate the Decision's content.>
- **Repo**: <repo/path>
- **Completion**:
