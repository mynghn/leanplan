...
9. **Self-check** before exit:
   ...
   - Graspability — a complex Decision's mechanism is graspable in-place via whatever aid fits (worked example, local diagram, or small table), not left as prose a reviewer must re-derive.
   ...
   - Proportionality — no Decision carries a split, example, or diagram a reviewer would read as redundant; resolution stays proportional to complexity (concise-not-compressed, `artifact-contract.md`).
...
- **Decision-level graspability aid.** The same "prefer a visual where prose would force re-derivation" applied at the single-Decision scale, complementing the mandatory Architecture visual above: when one Decision's mechanism is complex enough that prose alone would leave a reviewer re-deriving it — a non-obvious algorithm, a state transition, a control- or data-flow sequence, a field mapping — present that mechanism with an aid so the grasp lands in place. The aid is the author's choice — a worked example, a local diagram, a small table — and the contract is the reviewer's in-place grasp, not the presence of any particular aid. Unlike the Architecture visual it is not mandatory: it fires only on genuine mechanism complexity, gated by the Trivial-vs-non-trivial guard below.
- **Trivial vs. non-trivial.** Resolution is spent in proportion to a Decision's complexity, never as a uniform per-Decision tax — concise-not-compressed at the Decision level (`artifact-contract.md` → Prose Style). Trivial decisions get a one-line inline why and gain neither the atomicity split nor the graspability aid; non-trivial decisions (a real alternative was weighed, a tradeoff accepted, an invalidation trigger worth recording) anchor to Rationale, and earn a split or an aid only where genuine complexity needs it.
  - ✅ trivial — "store the new flag on the existing `accounts` row — the repo's standard for per-account state" (no live alternative → one line, no Rationale; one choice → no split; obvious mechanism → no aid).
  ...
  - ❌ a split or an aid on the trivial one — a second block or a diagram a reviewer reads as redundant, resolution the Decision never needed.
