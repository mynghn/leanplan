...
- `<cwd>/docs/features/<KEY>/requirements.md` (required). If absent, stop and tell the user to run `/leanplan-requirements <feature-title>` (Claude) or `leanplan-requirements <feature-title>` / `$leanplan requirements <feature-title>` (Codex) first — it allocates the `<KEY>` (`NNNN-slug`) dir and prints its path.
...
Next edge: `/leanplan-design <KEY>` (Claude) or `leanplan-design <KEY>` / `$leanplan design <KEY>` (Codex).
