...
2. **Derive Behavior items**: for each desired outcome in Requirements, ask what externally-observable behavior signals it. Write as `B-<N>: <slug>` under `## Behavior`. One item per behavior; don't fold two into one — the Behavior instance of inner-item atomicity (`artifact-contract.md` → One Concern Per Item).
...
- **Generic-category tech only.** Specific stack names → Design. Generic categories (message queue, event stream, HTTP API) are *expected* here; only a named stack is drift — a different policy from Requirements (`leanplan-validate` warns on named-product terms in the Spec, leaving the generic categories it legitimately uses untouched).
