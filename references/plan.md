...
- **Anchors carry ID + slug (identity, not restatement).** `SPEC#O-1-detected-anomaly-published-within-5s` — ID stable; slug names the reference at-a-glance. Don't paraphrase the item's content in the task card; rely on the anchor + JIT load (`artifact-contract.md` → One Prose Home Per Fact).
- **Forward coverage has one home.** Inline `Completion` citations are the canonical SPEC↔TASK mapping; a forward-coverage table, if you keep one, is a derived `**GAP**` view — not a re-authored mapping (`artifact-contract.md` → One Prose Home Per Fact).
