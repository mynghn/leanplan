...
- **Forward coverage has one home.** Inline `Completion` citations are the canonical SPEC↔TASK mapping; a forward-coverage table, if kept, is a derived `**GAP**` view (`artifact-contract.md` → One Prose Home Per Fact).
