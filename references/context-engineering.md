...
- **Grep-friendly anchored headings (ID+slug)** — `leanplan.md` §6; §8 / §9 → [[jit-loading]], [[literal-vs-latent-matching]]. The anchor is the lightweight pointer JIT resolves to full content; the grep-able exact slug is a literal lexical handle, not a latent-inference lookup.
