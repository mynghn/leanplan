...
- **Isolate breadth-heavy investigation** — `specify.md` / `design.md` / `implement.md` (wide research / code scans), and `tasks.md` (the bidirectional-verification sweep on large features) → [[context-isolation]], [[explore-then-compact-handoff]]. Fan wide reading into a sub-agent that returns only the distilled artifact or gap list.
- **Session-boundary discipline** — philosophy + `framework-design.md` §1; §13 harness note → [[explore-execute-boundary]], [[compaction-vs-eviction]], [[explore-then-compact-handoff]], [[context-isolation]]. Keep the planning spine warm in one session; hard-cut to a fresh frame at plan→implementation. The "warm session" half also rests on [[prefix-cache-economics]].
