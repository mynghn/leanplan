...
2. **No flat task scripting.** Task cards carry intent + constraints + anchors + completion criteria. They are not step-by-step edit recipes. Implementation agents re-derive against current code at task entry. (context-engineering: jit-loading, distractor-sensitivity)
...
5. **One-deployment scope.** A feature should fit one deployable increment. Oversized work is split out before it is absorbed into a sprawling plan.
...
- `context-engineering.md` — the name→concept map resolving the `(context-engineering: <slug>)` grounding hooks that trail the principles above to their context-engineering concept: the one-line gloss locally, and the full node (definition, sources, related edges) in the live context-engineering knowledge base on challenge. Load only when a hook is challenged.
