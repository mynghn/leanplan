...
1. **Allocate the feature.** Parse `$ARGUMENTS` and pick the id form, then let `leanplan-new` create the dir (it is the single allocator — never `mkdir` yourself). It prints the resolved `docs/features/<id>` path on stdout — **capture that path** and use it for every subsequent write; if it exits non-zero, stop (don't write). Then, when a `deferrals.md` already exists for the feature (a later stage parked a decision Requirements owns, surfaced now on re-entry), **drain deferrals** (load-bearing): surface each unresolved `Defer-<N>` addressed to Requirements and re-examine it against the current option space — non-binding, re-derive not replay; resolve in place (`references/deferral.md`).
