...
7. **Distill WHYs** at close-out — migrate each non-obvious WHY into the strongest durable form. **The constraint of each of the card's load-bearing citations migrates too** (the SPEC `O` / `INV` it realizes, the DESIGN `Decision` it builds on) — mandatory for that class, not gated on "non-obvious", since that substance is the step-2 consultation proof. Load `~/.local/share/leanplan/references/impl-closeout.md` for the tier hierarchy, the commit-message / PR-body promotion rules, and the close-out reconciliation that verifies this; JIT — fetch only now, at close-out, not before. (context-engineering: jit-loading)
