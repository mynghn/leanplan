...
- **No implementation choices.** No specific stack (Kafka, Redis, Postgres, gRPC), no internal architecture, no chosen pattern — no tech at all, generic or named (`leanplan-validate` warns on named-product terms; generic-category leakage is a review catch). Domain-vocabulary channels — "admin tool", "partner API", "batch integration" — are fine; they name channels, not choices.
