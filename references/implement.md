...
Mid-stage, if a disturbance shifts the understanding, `/leanplan-sharpen` (Claude) or `leanplan-sharpen` / `$leanplan sharpen` (Codex) is the sanctioned, opt-in response — an off-pipeline reflect-and-re-derive move that reads your artifacts but never edits them, distinct from the Stop-The-Line / Artifact Update Loop below, which deliberately walks up and edits the affected layer (via `/leanplan-revise` on Claude or `leanplan-revise` / `$leanplan revise` on Codex).
