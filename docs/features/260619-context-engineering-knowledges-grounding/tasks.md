# 260619-context-engineering-knowledges-grounding — Tasks
...
- **Completion**:
  ...
  - Each carries frontmatter `source: ce-kb:<slug>` + `last_refreshed: <ISO date>` — verifies `Spec#C-2-provenance-is-dated-and-visible`.
  - Every `Related: [[slug]]` edge resolves to a sibling node (no dangling) — supports `Spec#C-1-portable-self-contained`.
  - Each node defines the concept + cites its source(s) — supports `Spec#B-2-principle-resolves-to-grounded-definition`.
- **Dependencies**: none.

## T: A2

- **Goal**: Author the mapping `references/context-engineering.md` per `Design#D-1-two-layer-vendored-archive` — a conclusion-first index from each grounded principle to its `[[concept]]`, organized by the three axes (position / length / noise). Anchor; do not inline node content.
- **Repo**: `mynghn/leanplan` — `references/context-engineering.md`.
- **Completion**:
  - Every grounded principle in `research.md` → Principle → concept mapping appears with its `[[concept]]` link, each resolving to an A1 node — verifies the resolution half of `Spec#B-2-principle-resolves-to-grounded-definition`.
...
- **Goal**: Add `(CE: <slug>)` name-hooks to the pre-existing load-bearing rules in `philosophy.md` and `framework-design.md` (§1, §4, §6, §10) per `Design#D-2-surface-grounds-by-name-hook`, using the verified `research.md` → Principle → concept mapping. Names only — never inline content.
- **Repo**: `mynghn/leanplan` — `references/philosophy.md`, `framework-design.md`.
- **Completion**:
  - Each pre-existing load-bearing rule names the concept it implements, resolvable via the mapping — verifies `Spec#B-1-load-bearing-rule-names-its-principle`.
  - No inlined concept content added to surface docs — supports `Spec#C-3-grounding-stays-off-the-hot-path`.
...
- **Goal**: Add the two §6 write-time rules per `Design#D-5-stable-to-volatile-load-order` and `Design#D-6-edge-placement-in-long-artifacts`: stable→volatile load ordering (+ adapter-authoring note) and edge-placement past the §6 ToC>100-line threshold. Self-hook `(CE: prefix-cache-economics)` and `(CE: lost-in-the-middle)` respectively.
- **Repo**: `mynghn/leanplan` — `framework-design.md` §6 + adapter-authoring note.
- **Completion**:
  - §6 carries both rules; the edge-placement rule keys off the existing ToC>100-line threshold — verifies `Spec#B-3-framework-conforms-to-its-own-advice`. Both stay write-time guidance, not validator-enforced (no `validate.py` change).
- **Dependencies**: A2 (enabler — hooks resolve once the mapping lands).
...
- **Completion**:
  - With the external KB absent, every rule → principle → definition → related-concept reference resolves locally; `git diff` shows no edit to `install.sh` or `scripts/validate.py` — verifies `Spec#C-1-portable-self-contained`.
  - The default-loaded surface is enlarged only by `(CE: …)` name-hooks, not inlined content — verifies `Spec#C-3-grounding-stays-off-the-hot-path`.
...
|---|---|
| B-1 load-bearing-rule-names-its-principle | S1 (+ C1–C4 self-hooks) |
| B-2 principle-resolves-to-grounded-definition | A1 + A2 |
| B-3 framework-conforms-to-its-own-advice | C1 + C2 + C3 + C4 |
| C-1 portable-self-contained | V1 |
| C-2 provenance-is-dated-and-visible | A1 |
| C-3 grounding-stays-off-the-hot-path | V1 (realized by A1 + A2 + S1) |
