...
Forces: grounding must travel with the install (`C-1`), stay off the hot path (`C-3`), and not rot silently (`C-2`) — while the framework stays lean.

Alternatives considered:

- **Name-only, no vendored content** (Report 1's school) — ground purely by concept name, treat the KB as an optional external companion. Rejected: fails `C-1`/`B-2` — a reader on a fresh install can name the principle but cannot reach its definition+source. Names alone are not grounding; they are pointers to nothing shipped.
- **Inline the concept into the surface doc** — rejected: violates `C-3` (bloats the hot path) and the small-surface principle; it is the exact bloat the brief warns against.
- **Single flat `context-engineering.md`** holding map + all concepts — rejected: one large file defeats JIT (you load all 15 to read one) and muddles the mapping/definition layers. The two-layer split (map → nodes) mirrors LeanPlan's own surface→archive and lets a challenge load just the one node it needs.
- **Vendor only the cited subset (~10), trim edges** — rejected after audit: the `Related`-edge closure from the cited set reaches all 15 (the `three-axes-of-context-degradation` lens links all six axis members; `compaction-vs-eviction` pulls in two more; `explore-then-compact-handoff` pulls in `explore-execute-boundary`), so no proper subset is link-closed. A subset forces edge-pruning that diverges the copies from the KB and guts the lens. All 15 cost ≈ 0 (small distilled nodes) and keep fidelity. **This overturns the brief's "subset, not all 14" locked decision** — the premise (that a smaller closed set exists) was false.

Chosen: vendor all 15 as a two-layer archive. This is in-bounds under principle 7: P6/P7 govern transient per-feature plan docs under `docs/features/`, not the framework's permanent `references/` (corroborated — the validator and the no-frontmatter rule are both feature-scoped; `research.md` → Principle 7 scope).

Invalidation triggers: the KB's own concept set changes shape (new axis, merged concepts); a concept's `Related` edges change enough that the vendored copy misleads; the leanness budget is genuinely threatened by node count (not at 15). (This first trigger fired during vendoring on 2026-06-20: after the 2026-06-19 audit the KB gained `explore-execute-boundary` and `explore-then-compact-handoff` acquired an edge to it, so the closure was re-measured 14→15 and the full set re-vendored — `research.md` → CE knowledge base closure note.)
...
Vendoring buys portability at the cost of drift from the source KB. Mitigation mirrors the KB's own discipline against SOTA: stamp each node with `source` + `last_refreshed` so staleness is visible by inspection (`C-2`), and provide an optional regenerator.
