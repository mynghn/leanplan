...
Forces: the delta must survive a context reset (`C-4`), must not live in the committed surface artifacts (`C-1`), is a cross-stage *interpretation* (it can fire mid-requirement through implementation), and is meant to be consumed by downstream stages and the future C (#9). Homes considered:
...
Invalidation trigger: if C (#9) lands with a different consumption model (e.g. deltas inlined into a revision changelog), revisit whether a standalone archive is the right home. Validator support for the new artifact/anchor is an implementation task; if it proves heavy, entries can degrade to research-style descriptive headings without changing the design intent.

## D-3: reflect-verify-rederive-protocol

Forces: the move must sharpen, not thrash (`requirements` "bounded, never derailing"), and must not obey an injected claim on faith. Two non-obvious stances:
...
Boundary with implementation: `implement.md`'s existing Stop-The-Line + Artifact Update Loop already conflate cognitive re-derivation with artifact editing, but only at implementation. B factors out the *cognitive* half as a stage-agnostic move and deliberately does **not** touch implementation's loop or perform editing — generalizing and reconciling that loop is C's (#9) job.
