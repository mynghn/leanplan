...
- **Scope — reference-only, no new surface.** The change lives in the shared `references/requirements.md` (inherited by both the Claude `requirement` adapter and the Codex dispatcher); no new skill, reference doc, artifact, or tool grant — `AskUserQuestion` is already granted in `adapters/claude/requirement/SKILL.md`. A matching self-check item and a guardrail are added alongside the step.
