...
- **Stay off the user review surface** (`Spec#C-2-surface-no-less-lean`). Every edit lands in framework references (`impl.md`, `impl-closeout.md`, and at most a one-line review-facing pointer) — never in a feature's `requirement` / `spec` / `design` / `plan`. Add no new artifact kind, and write no round-local handle (`B-N` / `D-N` / `Spec#…`) into product code, commits, or PRs — the "substance, not the key" rule the feature itself enforces.
