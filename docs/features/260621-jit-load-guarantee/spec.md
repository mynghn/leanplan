...
### INV-1: archive-citation-never-force-loaded
...
### INV-2: surface-no-less-lean
The review surface (REQUIREMENT, SPEC, DESIGN, TASK) stays at least as lean as before this contract. Any consultation signal the contract introduces adds no standing prose a reviewer must read to grasp the feature, restates no archived or anchored content back onto the surface, and pushes no surface artifact over its existing Surface Budget.

## Non-goals
- **The form of the consultation signal is unspecified here.** Whether consultation is evidenced by a receipt, a restatement at the point of use, a check token, or a recorded trace is a DESIGN choice. This contract requires only that consultation be observable (O-1) and a skip surfaceable (O-2).
