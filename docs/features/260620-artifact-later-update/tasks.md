...
Tracks: **F** Foundation (shared machinery the skill builds on), **R** the `/revise` skill, **M** Migration (re-home implementation's loop). Edges are enablers — the implementation agent re-evaluates them at task entry.
...
- **Dependencies**: none.
- **Guidelines**: lowest-stakes card — `Design#D-6-resolve-understanding-citations` marks it droppable; if scope tightens at implementation, cut this (C-1 stays covered by R1's gate).
...
- **Completion**:
  - (a) invoking `/revise` at requirement, spec, design, plan, and implementation — including mid-task — engages on the feature's committed artifacts (`Spec#B-1-update-invocable-from-any-in-flight-stage`);
...
- **Goal**: Collapse the implementation Artifact Update Loop's editing core into `/revise` so during-implementation drift uses the one unified entry (`Spec#B-1-update-invocable-from-any-in-flight-stage`) — implementation's six stop-the-line triggers stay as implementation-time detections but delegate the edit-and-propagate walk to `/revise`, per `Design#D-1-revise-unified-editing-entry`.
- **Repo**: `leanplan` — `references/implement.md`
- **Completion**:
  - implementation's stop-the-line section hands off to `/revise` for the artifact edit instead of describing its own inline walk-up; a mid-implementation drift and a between-stage drift follow the same documented path (`Spec#B-1-update-invocable-from-any-in-flight-stage`).
- **Dependencies**: R1 lands the `/revise` entry implementation delegates to.
- **Guidelines**: if re-homing grows past a doc re-point (implementation's loop carries implementation-specific nuance worth keeping), land R1 + Foundation first and treat this as the fast-follow the `design-rationale.md#D-1-revise-unified-editing-entry` scope note sanctions.
