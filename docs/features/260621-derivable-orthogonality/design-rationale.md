...
**Forces.** The model must let the two high-traffic seams (REQ↔Spec, Design↔Tasks) be *derived*, not memorized (`Spec#B-1`, `Spec#B-2`), while staying minimal — no axis that doesn't earn its slot (`Spec#C-2`).
...
**Forces.** `Spec#B-3` wants every element name to *follow from the model*; the anti-churn bar (§8/§9, which rejected the `<KEY>` sweep at ~70 sites) and `Spec#C-2` bound the cost. The user elected *ideal-shape names across the board*, accepting the churn — so the job became: get the scheme right, then isolate the execution.

**The naming rule.** A skill shares its artifact's root — a verb where English has one (`specify`→Spec, `design`→Design), the bare noun where it doesn't (`requirements`→Requirements, `tasks`→Tasks). This retires the old `plan`-edge / `Tasks`-artifact / `tasks.md`-file triple (three names for one element) and makes `requirements`/`tasks` principled noun-skills rather than exceptions.
