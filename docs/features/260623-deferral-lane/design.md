...
flowchart TB
    ...
    REC["validate.py advisory check"]
...
- **Where** the *forward* disposal points where a stage discards off-altitude content bound for a later stage — Requirements "strip them", Spec "push to Design", Design "belongs in Tasks". (Tasks is the last planning stage; its "push to Design" is backward — a `revise` drift, not a deferral.) Each gains an affirmative redirect: *when a genuine cross-stage decision surfaces, append a `Defer-N` addressed forward to the later stage that owns it instead of discarding it* — the positive-instruction complement of the existing prohibition.
...
Each *downstream* stage's entry step ("Load …") — Spec, Design, Tasks — is extended to drain the deferrals addressed to it (Requirements is the first stage, so nothing is ever addressed back to it). Satisfies `Spec#B-2-deferrals-surfaced-at-owning-stage`, `Spec#C-3-surfaced-deferral-is-non-binding`.
...
- **Resolved needs no separate hunt.** A drained deferral's `(resolved -> …)` marker cites where it landed — a `Spec#` / `Design#` / `Tasks#` anchor that is itself resolution-checked and tracked by that stage's normal coverage. Once resolved, follow the decision it became, not the deferral; no review-tier deferral obligation is added.
- Why advisory-only (the dropped close-out substance tier): rationale at [design-rationale.md#D-5-no-loss-via-advisory-check].
