...
- **Substance crosses, the key doesn't** — when a durable artifact needs a round's WHY, it states the constraint in plain terms ("tokens must never be stored in clear"), never the opaque handle ("satisfies `C-1`"). The reason is durable; the round-local key for it is not.
...
- **No change to the anchor scheme** — no new id format, no renaming of `B-` / `C-` / `D-` or of feature ids.
