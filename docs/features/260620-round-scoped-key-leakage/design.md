...
A shared detector, `scan-leaks` (sibling of `validate.py`), flags the high-precision token class only — round-scoped anchor tokens and cross-artifact citations — leaving feature ids and readable labels alone. Rationale at [design-rationale.md#D-2-narrow-leak-detector](design-rationale.md#D-2-narrow-leak-detector).

- **Detected** (two alternations):
  - Bare anchor tokens — `\b(B|C|Delta|D)-\d+\b` (e.g. `C-1`, `D-2`).
  - Cross-artifact citations — `\b(Spec|Design|Tasks|Rationale|Research|Understanding)#[A-Za-z0-9:_-]+` — the `:` covers `T:`-style ids (e.g. `Spec#C-1-…`, `Design#D-2-…`). Requirements is omitted: it carries no citable anchors, so it appears in neither `validate.py`'s citation kinds nor the contract.
- **Not detected** (narrow by construction) — feature-id forms (`NNNN-slug`, `YYMMDD-slug`, bare tracker keys), readable scope labels (`docs(<slug>)`), bare numbers. None match the pattern, so the grey zone needs no special-casing. Two sub-cases: scope labels and tracker links are genuinely Spec-allowed; a feature id used as a standing concept *is* Spec-prohibited but is deliberately left to the guidance arm — too low-precision for a mechanical gate without false positives (see rationale).
...
Wire the detector into three surface-specific integration points; leave other durable surfaces to guidance. Rationale at [design-rationale.md#D-3-per-surface-backstop](design-rationale.md#D-3-per-surface-backstop).
...
- **Verification** — `scan-leaks` correctness is held by `leanplan-selftest` defect injection (a fixture leaking `C-1` into a non-artifact file must be caught).
- Realizes the mechanical arm of `Spec#C-1-durable-artifacts-free-of-round-scoped-keys`.
