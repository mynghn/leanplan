...
The failure mode is information compression wearing concision's clothes. Chasing brevity, an author chains distinct concepts into a separator-joined noun pile — e.g. `call order · bypass-check save · conflict-id union → reconciled state`. It reads as terse, but the reviewer cannot judge it without first decompressing each token back into the concept it stands for. That hidden decompression cost is what erodes review fidelity — and it is invisible, because a compressed line looks like a lean model artifact, so the reviewer skims past it.
...
The shared, all-artifact prose guidance distinguishes *concise* from *compressed* — naming information-compression-as-false-brevity as a failure mode the author can self-catch and the reviewer can reject by name. Brevity is defined explicitly: short *explanatory* sentences and lists, never fewer tokens bought by packing several concepts into one separator-joined token.

User stories:

- **Author self-catches the noun pile** — tempted to pack several concepts into one token, the author meets a named rule that says spend the words instead.
