...
- Shape: discovery is **multi-channel**, not a single human-question move — the Requirements draw-out's `AskUserQuestion` mirror was the first draft, corrected at implementation (`UnderstandingShifts#Delta-1-probe-discovers-by-investigation-not-elicitation`). Three channels: **inspect the as-is** (code, architecture, live environment — reality authoritative), **research the outer world** (SOTA, prior art, convention for the boundary class — the *generative reach*, since as-is inspection alone re-derives the status quo), and **ask the planner** (`AskUserQuestion` on Claude / runtime-native prompt on Codex) for the human-held slice + accept/decline. Probe classes: failure modes, concurrency, observable error behavior, environmental couplings. **Altitude-bounded** (so the probe's investigation does not overlap Design's): the channels discover *what the boundary observably exposes* (contract facts, alternative-free), not the realization — a finding that is a choice among realizations routes to Design via Specify's existing Spec test / *No false optionality*. Same act as Design's inspection, different output altitude (contract vs realization); research is continuous across the two edges with one shared `research.md` archive. Reuses Specify's existing research machinery (`research.md` as a source, the *Isolate breadth-heavy research* sub-agent guardrail) — no new surface; named techniques stay in the archive, not the stage doc.
