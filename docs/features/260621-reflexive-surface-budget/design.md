...
The always-loaded hot path gains a within-reference second tier: step-scoped reference detail moves to an on-demand companion, reusing the framework's existing "load X at <trigger>" idiom. The one clear application is implement.md's three close-out tables.
...
flowchart TD
    ...
    subgraph ondemand["On-demand companions (loaded at consuming step)"]
      ...
      C2["philosophy.md"]
      NEW["implement-closeout.md (NEW: distillation + commit tables)"]
    end
    R -.->|"before writing structure"| C1
    R -.->|"when a principle is in question"| C2
    R ==>|"implementation step 7: at close-out"| NEW
```

Caption: the existing tiering — adapter + reference always-loaded, shared companions on-demand — is unchanged. The new edge is the bold one: implement.md's close-out tables become `implement-closeout.md`, loaded by a pointer at implementation procedure step 7. Worked examples and templates stay inline (no reliable procedural trigger). Realizes `Spec#B-2-deferred-content-not-always-loaded` and `Spec#C-1-deferral-preserves-guidance`.
...
## D-2: defer-implementation-closeout-tables

Move implement.md's three close-out tables — Distillation Hierarchy, Commit-message-vs-inline-comment, Squash-durability-promotion (implement.md ~L63–97, ~35 lines / 33%) — verbatim into a new `references/implement-closeout.md`. They are consumed only at close-out (procedure steps 7–8, 10), and implementation is the scarcest-window stage (it also holds current code + JIT-loaded anchors + the card), so trimming them has the highest marginal value of any reference. Realization:

- implement.md procedure step 7 changes from "per the hierarchy below" to a load-pointer citing the absolute runtime path `~/.local/share/leanplan/references/implement-closeout.md` (CE: jit-loading). One load at step 7 serves steps 8 and 10 (all close-out). The *instruction* to distill stays inline; only the lookup tables defer.
...
implement.md drops 106 → ~71 always-loaded lines. Realizes `Spec#B-2-deferred-content-not-always-loaded`; the step-7 pointer is the resolvable load-point for `Spec#C-1-deferral-preserves-guidance`. See rationale at [design-rationale.md#D-2-defer-implementation-closeout-tables].
