...
- **Input** — `<cwd>/docs/features/<KEY>/spec.md`; if absent, stop and point the user to `leanplan-specify` or `$leanplan specify`.
- **Validate** — `python3 ~/.local/share/leanplan/scripts/validate.py <cwd>/docs/features/<KEY> --stage design`.
- **Hand off** — next move is `leanplan-tasks <KEY>` or `$leanplan tasks <KEY>`.
