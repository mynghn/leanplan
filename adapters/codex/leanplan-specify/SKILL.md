...
Load `references/specify.md` from `LP_ROOT` — it is authoritative for the procedure, generic-category tech guard, B/C split, Research archive rule, and contract-boundary probe (inspect/research/ask, opt-in). Load these on demand, not up front (context-engineering: jit-loading):
