...
- **Validate** — after emitting a delta, run `python3 ~/.local/share/leanplan/scripts/validate.py docs/features/<KEY>` to confirm its scope-of-impact citations resolve. If you paused **before all surface artifacts exist** (mid-requirement/spec/design), scope `--stage` to the in-flight stage so validation fails on the delta, not on not-yet-authored downstream files; mid-plan or mid-impl the full surface exists, so omit `--stage` (there is no `impl` stage value).
