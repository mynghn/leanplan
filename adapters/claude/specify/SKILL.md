...
Load `~/.local/share/leanplan/references/specify.md` — it is authoritative for the procedure ("what a SPEC is NOT" test, generic-category tech guard, RESEARCH archive rule); this adapter only wires the stage to the Claude Code runtime. Load these on demand, not up front (CE: jit-loading):
