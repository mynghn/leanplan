...
Load `~/.local/share/leanplan/references/specify.md` — the stage procedure ("what a SPEC is NOT" test, generic-category tech guard, RESEARCH archive rule). Load these on demand, not up front (CE: jit-loading):
...
If `<cwd>/docs/features/<KEY>/requirement.md` is missing, stop and tell the user to run `/requirement <feature-title>` first (it allocates and prints the `<KEY>` dir). Validate after writing with `python3 ~/.local/share/leanplan/scripts/validate.py <cwd>/docs/features/<KEY> --stage spec`.
