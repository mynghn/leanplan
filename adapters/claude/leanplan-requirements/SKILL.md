...
- **Allocator** — `"$LP_ROOT/scripts/leanplan-new"` is the single directory allocator: capture its stdout path, stop on non-zero, never `mkdir`. The id-form choice (sequence / tracker-key / date) lives in `requirements.md` Procedure step 1.
