...
argument-hint: "[brief biz intent | NNNN-slug to revise]"
...
Tools above are Claude Code-specific. Resolve the feature by running `~/.local/share/leanplan/scripts/leanplan-new "<slug-or-title>"` to allocate `docs/features/<NNNN-slug>` (capture the path it prints on stdout; stop if it exits non-zero), then write `requirement.md` there — do not `mkdir` yourself. Jira / Slack MCP tools remain for *optional upstream context* per `requirement.md` Procedure step 2 when the user references a tracker — recorded under `## Upstream`, never used as the directory name. Validate at any time against the captured path, e.g. `python3 ~/.local/share/leanplan/scripts/validate.py docs/features/0007-anomaly-publisher --stage requirement`.
