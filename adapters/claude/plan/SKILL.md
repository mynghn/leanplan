...
Load `~/.local/share/leanplan/references/plan.md` — it is authoritative for the procedure + template (track-prefixed task IDs P/A/D/I, enabler-not-gate dependency wording, bidirectional verification, advisory one-deployment guardrail); this adapter only wires the stage to the Claude Code runtime. Load these on demand, not up front (CE: jit-loading):
