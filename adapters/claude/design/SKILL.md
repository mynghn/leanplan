...
Load `~/.local/share/leanplan/references/design.md` — it is authoritative for the procedure + template (Architecture-first rule, non-trivial-only rationale anchoring, SPEC O/INV coverage check with TASK-direct realization path for trivial items); this adapter only wires the stage to the Claude Code runtime. Load these on demand, not up front (CE: jit-loading):
...
Runtime glue only (the reference carries the "inspect current code first" rule and the rest of the procedure):
